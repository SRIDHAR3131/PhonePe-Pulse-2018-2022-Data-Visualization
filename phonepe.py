...
map_user= pd.read_sql_table('map_user', conn) #2
...
with st.sidebar:
    main = option_menu(None, ["Home", 'Visualization'],icons=['house', 'pin-map'], menu_icon="cast", default_index=1)


if main=='Home':
    image = Image.open("PhonePe_Logo_full.png")
    st.image(image,width=500)
    st.markdown("#### Welcome to the **:violet[PhonePe Pulse]** Data Visualization! ")
    st.markdown("This visualization allows you to explore and analyze PhonePe's pulse data from 2018 to 2022. With interactive charts and various metrics to choose from, you can gain insights into PhonePe's business performance and growth over time.")
    ...
    st.markdown("This tool is built using  *:red[Python]* for scripting,  *:blue[Streamlit]* for creating GUI, *:green[Plotly Express]* for interactive data visualization tool like bar chart,line chart,sunburst chart,scatter geo, choropleth map etc.. and  *:orange[MySQL Database]* for store and read the data by raising query."
 ...
 # navigation menu
    ...
    if analyser == "Transaction":
        ...
        selected = option_menu(
            ...
            icons=["pencil-fill", "bar-chart-fill"],  # https://icons.getbootstrap.com/"currency-rupee", "fill-person-fill"
        ...
        with tran:
            ...
            total['Amount'] = total['Transaction_amount'].apply(convert_to_crore)
            st.markdown('#### Transaction amount')
            ss = st.write(total[['Amount']].style.hide(axis="index").to_html(), unsafe_allow_html=True)
        with count:
            total_amt = agg_tran.loc[(agg_tran['Year'] == Year) & (agg_tran['Quarter'] == Quarter)]
            total = total_amt.groupby(['Year'], as_index=False).sum(numeric_only=True)
            total['Transaction'] = total['Total_transaction'].apply(convert_to_crore1)
    ...
    #===================================GEO MAP TRANSACTION AMOUNT ==============================================================
        if selected=='TRANSACTION AMOUNT':
            #FYI data preprocessing take place for missing row vales!!!!!!!!!
            map_tran.loc[len(map_tran)] = ['Dadra and Nagar Haveli and Daman and Diu', 'unknow district', '2018','Q3 (Jul - Sep)', 0, 0]
            map_tran.loc[len(map_tran)] = ['Dadra and Nagar Haveli and Daman and Diu', 'unknown district', '2018','Q1 (Jan - Mar)', 0, 0]
            map_tran.loc[len(map_tran)] = ['Dadra and Nagar Haveli and Daman and Diu', 'unknow district', '2018','Q1 (Jan - Mar)', 0, 0]
            map_tran.loc[len(map_tran)] = ['Dadra and Nagar Haveli and Daman and Diu', 'unknow district', '2018','Q2 (Apr - Jun)', 0, 0]
            map_tran.sort_values('State', ignore_index=True)
            ...
            dt_scatter = map_t.groupby(['State', 'District'], as_index=False).sum(numeric_only=True)
            ...
            # district trace
            amount = []
            for i in dt_scatter['Transaction_amount']:
                amount.append(i)
            dt0['Transaction_amount'] = amount
            ...
            state.update_traces(marker=dict(color="white", size=0.3))
            state.update_geos(fitbounds="locations", visible=False)
            #using scatter geo for  districts trace
            dist= px.scatter_geo(dt0,
                                    lon=dt0['Longitude'],
                                    lat=dt0['Latitude'],
                                    size=dt0['Transaction_amount'],
                                    hover_name="District",
                                    hover_data=["State", 'Transaction_amount'],
                                    title='District',
                                    size_max=20)
            dist.update_traces(marker=dict(color="red", line_width=.15))
            ...
            map_data.add_trace(dist.data[0])
            ...
            fig1 = px.sunburst(dist_tran, path=['State', 'District'], values='Transaction_amount',title="DISTRICT TRANSACTION AMOUNT")
            with district:
            ...
            with top:
                ...
                top10_tran_amount['Transaction_amount'] = top10_tran_amount['Transaction_amount'].apply(convert_to_crore)
                top10_dt_tran_amount['Transaction_amount'] = top10_dt_tran_amount['Transaction_amount'].apply(convert_to_crore)
            ...
            mode,pie1= st.columns([2,1])
            # payment instrument
            with mode:
                payment = agg_tran.loc[(agg_tran['Year'] == Year) & (agg_tran['Quarter'] == Quarter) & (agg_tran['State'] == State)]
                payment['Transaction_amount'] = payment['Transaction_amount'].apply(convert_to_crore2)
                fig = px.bar(payment, y='Transaction_type', x='Transaction_amount', color='Transaction_amount',title='TYPE OF TRANSACTION', color_continuous_scale='spectral')
                st.plotly_chart(fig, use_container_width=True)

            #pie chart transaction
            with pie1:
                state_data = agg_tran.loc[(agg_tran['State'] == State)]
                pie_1 = state_data.groupby(['Year', 'State'], as_index=False).sum(numeric_only=True)
                pie_1['Transaction_amount'] = pie_1['Transaction_amount'].apply(convert_to_crore2)
                fig = go.Figure(data=[go.Pie(labels=pie_1['Year'], values=pie_1['Transaction_amount'],
                                                 pull=[0.2, 0, 0, 0, 0], marker_colors=px.colors.qualitative.Prism_r,
                                                 title=State,
            ...
            with line[0]:
                ...
                line = line.sort_values('Transaction_amount', ascending=True, ignore_index=True, axis=0)
                line['year'] = line['Year'] + line['Quarter']
                line['Transaction_amount'] = line['Transaction_amount'].apply(convert_to_crore2)
                fig = go.Figure(data=go.Scatter(x=line['year'], y=line['Transaction_amount'], text='Transaction_amount'))
                fig.update_layout(title='OVERALL TRANSACTION ')
                st.plotly_chart(fig, use_container_width=True)
    ...
    # ===================================GEO MAP TRANSACTION COUNT ==============================================================
            ...
            # district trace
            count = []
            for i in dt_scatter['Total_transaction']:
                count.append(i)
            dt0['Total_transaction'] = count

            # state trace
            state_0= px.scatter_geo(st_code0,
                                     lon=st_code0['Longitude'],
                                     lat=st_code0['Latitude'],
                                     text=st_code0['code'],  # code of state taken from online sources
                                     hover_name="state",
            ...
            # districts trace
            dist_0 = px.scatter_geo(dt0,
                                    lon=dt0['Longitude'],
                                    lat=dt0['Latitude'],
                                    size=dt0['Total_transaction'],
                                    hover_name="District",
                                    hover_data=["State", 'Total_transaction'],
                                    title='District',
                                    size_max=20)
            dist_0.update_traces(marker=dict(color="lightcyan", line_width=.15))
            # using choropleth map for plod add trace of state and district
            ...
            map_data_0.add_trace(dist_0.data[0])
            ...
            with district0:
                ...
                dist_tran1['Total_transaction'] = dist_tran1['Total_transaction'].apply(convert_to_lakh2)
            ...
            with state:
                ...
                stt_count = stt_count.sort_values('Total_transaction', ascending=False, ignore_index=True, axis=0)
                stt_count['Total_transaction'] = stt_count['Total_transaction'].apply(convert_to_crore2)
                figg2 = px.line(stt_count, x='State', y='Total_transaction', title='STATE TRANSACTION', height=650)
            ...
            with top:
                ...
                top10_tran_amount['Total_transaction'] = top10_tran_amount['Total_transaction'].apply(convert_to_crore1)
                top10_dt_tran_amount['Total_transaction'] = top10_dt_tran_amount['Total_transaction'].apply(convert_to_crore1)
            ...
            pay, pie2 = st.columns([2, 1])
            with pay:
                pay_count = agg_tran.loc[(agg_tran['Year'] == Year) & (agg_tran['Quarter'] == Quarter)]
                pay_mod = pay_count.groupby(['Year', 'Quarter', 'State', 'Transaction_type'], as_index=False).sum(numeric_only=True)
                pay_mod['Total_transaction'] = pay_mod['Total_transaction'].apply(convert_to_crore2)
                fig = px.bar(pay_mod, x="Transaction_type", y="Total_transaction",color="Transaction_type", barmode='overlay', title='TYPE OF TRANSACTION')
                st.plotly_chart(fig, use_container_width=True)
            ...
            with pie2:
                state_data = agg_tran.loc[(agg_tran['State'] == State)]
                pie_1 = state_data.groupby(['Year', 'State'], as_index=False).sum(numeric_only=True)
                pie_1['Total_transaction'] = pie_1['Total_transaction'].apply(convert_to_crore2)
                fig = go.Figure(data=[go.Pie(labels=pie_1['Year'], values=pie_1['Total_transaction'],
                                                 pull=[0.2, 0, 0, 0, 0], marker_colors=px.colors.qualitative.Set1,
                                                 title=State,
                                                 hole=0.6, showlegend=False)])
                st.plotly_chart(fig, use_container_width=True)
            ...
            with line[0]:
                ...
                line['year']=line['Year']+line['Quarter']
                line['Total_transaction'] = line['Total_transaction'].apply(convert_to_crore2)
                fig = go.Figure(data=go.Scatter(x=line['year'], y=line['Total_transaction'], text='Total_transaction'))
                fig.update_layout(title='OVERALL STATE TRANSACTION ')
                st.plotly_chart(fig, use_container_width=True)
    ...
    # ==============================overall register and app opened user================================================
        ...
        with reg:
            ...
            total_r['Register'] = total_r['Register'].apply(convert_to_crore1)
        ...
        with app:
            ...
            total_a['App_opened'] = total_a['App_opened'].apply(convert_to_crore1)
    ...
    #==================================REGISTER USER ANALYSIS==============================================================
        if selected=="REGISTER USER":
            ...
            dist_1 = px.scatter_geo(dt0,
                                    ...
                                    title='District',
                                    size_max=20)
            dist_1.update_traces(marker=dict(color="darkred", line_width=.15))
            # using choropleth map for plod add trace of state and district
            ...
            with dist3:
                ...
                dt_reg['Register'] = dt_reg['Register'].apply(convert_to_lakh2)
            ...
            with reg:
                ...
                reg0 = reg.sort_values('Register', ascending=False, ignore_index=True, axis=0)
                fig = px.bar(reg0, x='Register', y='State', color='Register', color_continuous_scale='dense',height=600,title='STATE REGISTER USER')
                st.plotly_chart(fig,use_container_width=True)
            ...
            with top:
                ...
                top10_reg_user['Register'] = top10_reg_user['Register'].apply(convert_to_lakh1)
                top10_dt_reg_user['Register'] = top10_dt_reg_user['Register'].apply(convert_to_lakh1)
                top10_pincode['Register'] = top10_pincode['Register'].apply(convert_to_lakh1)
            ...
            with line[0]:
                ...
                line = line.groupby(['Year', 'Quarter', 'State'], as_index=False).sum(numeric_only=True)
                line = line.sort_values('Register', ascending=True, ignore_index=True, axis=0)
                line['year'] = line['Year'] + line['Quarter']
                line['Register'] = line['Register'].apply(convert_to_lakh2)
                fig = go.Figure(data=go.Scatter(y=line['year'], x=line['Register'], text='Register'))
                fig.update_layout(title='OVERALL STATE REGISTER USER  ')
                st.plotly_chart(fig, use_container_width=True)
    ...
    #===================================APP OPENED USER ANALYSIS===========================================================
            ...
            group,top=st.columns([3,.6])
            with group:
                ar_user = agg_user.loc[(agg_user['Year'] == Year) & (agg_user['Quarter'] == Quarter)]
                ap_rg = ar_user.groupby(['Year', 'Quarter', 'State'], as_index=False).sum()
                app_reg = ap_rg.sort_values('Register', ascending=True, ignore_index=True, axis=0)
                app_reg1 = app_reg.sort_values('App_opened', ascending=True, ignore_index=True, axis=0)
                app_reg1['Register'] = app_reg1['Register'].apply(convert_to_lakh1)
                app_reg1['App_opened']=app_reg1['App_opened'].apply(convert_to_lakh1)
                ...
                dt = [t1, t2]
                layout = go.Layout(barmode='relative',height=600,title='REGISTER AND APP OPENED USER')
                compare = go.Figure(data=dt, layout=layout)

                group = st.columns(1)
                st.plotly_chart(compare, use_container_width=True)
            with top:
                st.write('###### TOP 10 APP USER')
    ...
    #=========================================DEVICE USER ANALYSIS=================================================================
            ...
            with state:
                ...
                st_dev['Total_User'] = st_dev['Total_User'].apply(convert_to_lakh2)
            ...
            with top:
                ...
                top_device_user = device_user.groupby(['Year','Quarter','Device_user'], as_index=False).sum(numeric_only=True)
                top10_device_per = top_device_user.sort_values(['User_percent'], ascending=False, ignore_index=True, axis=0)
                top10_device_per['User_percent'] = top_device_user['User_percent'].apply(convert_to_per)
